<!-- ============================================================
     ...
     Purpose: Full form to create a new customer
     Window ID: finance-create-customer
     Logic:
       - Multiple phone numbers (add/remove)
       - Multiple vehicle numbers (add/edit/remove)
       - Auto-create ledger with type=Receivables on save
     ============================================================ -->
...
<script lang="ts">
  ...
  function addVehicleNumber() { vehicleNumbers = [...vehicleNumbers, '']; }
  function removeVehicleNumber(i: number) { vehicleNumbers = vehicleNumbers.filter((_, idx) => idx !== i); }

  async function handleSave() {
    if (!name.trim()) { saveError = 'Name is required'; return; }

    const validPhones = phones.map(p => p.trim()).filter(p => p);
    const validVehicles = vehicleNumbers.map(v => v.trim()).filter(v => v);

    saving = true;
    saveError = '';
    saveSuccess = '';

    // 1. Get Receivables ledger type
    ...
    if (ltErr || !ltData) {
      saving = false;
      saveError = 'Could not find Receivables ledger type';
      return;
    }
    ...
    if (custErr || !custData) {
      saving = false;
      saveError = 'Failed to create customer: ' + (custErr?.message || '');
      return;
    }
    ...
    if (validPhones.length > 0) {
      const phoneRows = validPhones.map(phone => ({ customer_id: custData.id, phone }));
      await supabase.from('customer_phones').insert(phoneRows);
    }

    // 6. Save vehicle numbers
    if (validVehicles.length > 0) {
      const vRows = validVehicles.map(vehicle_number => ({ customer_id: custData.id, vehicle_number }));
    ...
    // Reset
    name = ''; place = ''; gender = '';
    phones = ['']; vehicleNumbers = [''];

    setTimeout(() => (saveSuccess = ''), 3000);
  }

  function handleCancel() {
    windowStore.close('finance-create-customer');
  }
</script>
  ...
  <div class="form-body">
    ...
    <div class="form-card">
      ...
      <div class="multi-section">
        ...
        {#each vehicleNumbers as vn, i}
          <div class="multi-row">
            <input type="text" bind:value={vehicleNumbers[i]} placeholder="Vehicle number" />
...
<style>
  .create-window { width:100%; height:100%; display:flex; flex-direction:column; background:#fafafa; box-sizing:border-box; }

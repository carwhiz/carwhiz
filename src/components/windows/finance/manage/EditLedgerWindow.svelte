<!-- ============================================================
     ...
     Purpose: Edit an existing ledger entry
     Window ID: finance-edit-ledger-{ledgerId}
     ============================================================ -->
...
<script lang="ts">
  ...
  onMount(async () => {
    await Promise.all([loadLedgerTypes(), loadLedgerCategories(), loadExpenseCategories()]);
    await loadLedger();
  });
  ...
  async function loadLedger() {
    ...
    const { data, error } = await supabase
      .from('ledger')
      .select('id, ledger_name, ledger_type_id, ledger_category_id, expense_category_id, bank_account_number, opening_balance, status')
      .eq('id', ledgerId)
      .single();

    loading = false;
    if (error || !data) { saveError = 'Failed to load ledger'; return; }
  ...
  async function handleSave() {
    ...
    const { error } = await supabase.from('ledger').update(row).eq('id', ledgerId);
    saving = false;

    if (error) { saveError = error.message; return; }

    saveSuccess = 'Ledger updated successfully!';
    setTimeout(() => (saveSuccess = ''), 3000);
  }

  function handleCancel() {
    windowStore.close('finance-edit-ledger-' + ledgerId);
  }
</script>
...
<div class="create-window">
  ...
  <div class="form-body">
    {#if loading}
      <div class="loading-msg">Loading ledger...</div>
    {:else}
      {#if saveError}
      ...
      <div class="form-card">
        ...
        <div class="form-row two-col">
          <div class="field">
            ...
            <select id="l-status" bind:value={status}>

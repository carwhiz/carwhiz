-- Migration 025: Multi-punch attendance system
-- ==============================================
-- Allow multiple check-in/check-out pairs per user per day.
-- Each scan creates or completes a punch pair.
-- Total worked hours = sum of all completed pairs.

-- 1. Drop the one-row-per-day unique constraint
ALTER TABLE public.attendance DROP CONSTRAINT IF EXISTS attendance_user_id_date_key;

-- 2. Add a punch_order column to track sequence within a day
ALTER TABLE public.attendance ADD COLUMN IF NOT EXISTS punch_order INT DEFAULT 1;

-- 3. Replace the fn_attendance_punch function

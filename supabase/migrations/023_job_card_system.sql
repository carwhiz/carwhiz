-- Migration 023: Job Card System
-- ===============================
-- Tables: job_cards, job_card_items, job_card_photos, job_card_notes, job_card_logs
-- Sequence: job_card_seq

-- 1. Sequence for job card numbers
...
-- 7. Audit triggers for all job card tables
...
CREATE TRIGGER trg_audit_job_card_items
  AFTER INSERT OR UPDATE OR DELETE ON public.job_card_items
  FOR EACH ROW EXECUTE FUNCTION public.fn_audit_log();

DROP TRIGGER IF EXISTS trg_audit_job_card_photos ON public.job_card_photos;
CREATE TRIGGER trg_audit_job_card_photos
  AFTER INSERT OR UPDATE OR DELETE ON public.job_card_photos
  FOR EACH ROW EXECUTE FUNCTION public.fn_audit_log();

DROP TRIGGER IF EXISTS trg_audit_job_card_notes ON public.job_card_notes;
CREATE TRIGGER trg_audit_job_card_notes
  AFTER INSERT OR UPDATE OR DELETE ON public.job_card_notes
  FOR EACH ROW EXECUTE FUNCTION public.fn_audit_log();

DROP TRIGGER IF EXISTS trg_audit_job_card_logs ON public.job_card_logs;
CREATE TRIGGER trg_audit_job_card_logs
  AFTER INSERT OR UPDATE OR DELETE ON public.job_card_logs
  FOR EACH ROW EXECUTE FUNCTION public.fn_audit_log();

-- 8. Storage bucket for job card photos
INSERT INTO storage.buckets (id, name, public)
VALUES ('job-card-photos', 'job-card-photos', true)
ON CONFLICT (id) DO NOTHING;

-- Allow public read/write for job card photos bucket
DROP POLICY IF EXISTS "job_card_photos_public_read" ON storage.objects;
CREATE POLICY "job_card_photos_public_read" ON storage.objects
  FOR SELECT USING (bucket_id = 'job-card-photos');
DROP POLICY IF EXISTS "job_card_photos_public_insert" ON storage.objects;
CREATE POLICY "job_card_photos_public_insert" ON storage.objects
  FOR INSERT WITH CHECK (bucket_id = 'job-card-photos');
DROP POLICY IF EXISTS "job_card_photos_public_delete" ON storage.objects;
CREATE POLICY "job_card_photos_public_delete" ON storage.objects
  FOR DELETE USING (bucket_id = 'job-card-photos');

-- 9. RPC: List users (avoids direct REST GET on users table which can cause 400 errors)
